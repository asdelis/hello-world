...
int main() {
    ...
    //testing min element
    int min = *std::min_element(v6.begin(), v6.end());
    assert(min == 1);
    std::cout << min << "\n";
    ...
    //testing count_if
    int count = std::count_if(v6.begin(), v6.end(), [](int i) { return i % 2 == 0; });
    std::cout << count << "\n";
    
    //testing remove_if
    //ask about how to use pop_back to this
    std::remove_if(v6.begin(), v6.end(), [](int i) { return i % 2 == 0; });
    for (int x : v6){
        std::cout << x;
    }
    std::cout << "\n";
